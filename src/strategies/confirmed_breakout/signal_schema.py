"""Confirmed Breakout strategy-owned SignalFrame schema (versioned SSOT).
...
def get_signal_frame_schema(strategy_version: str) -> SignalFrameSchemaV1:
    ...
    if strategy_version not in SCHEMAS:
        raise ValueError(
            f"Unknown confirmed_breakout schema version '{strategy_version}'. Available: {sorted(SCHEMAS.keys())}"
        )
