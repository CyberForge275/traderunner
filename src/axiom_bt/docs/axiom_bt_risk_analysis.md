...
**Problem:**
`full_backtest_runner` loads extra historical bars (`warmup_days`) for indicator calculation. If strategy emits signals during warmup period, those signals use data **outside requested backtest window**.

**Code:**
```python
# File: full_backtest_runner.py, lines 252-263
...
```python
# File: full_backtest_runner.py, lines 354-356
...
**File:** `full_backtest_runner.py`, lines 294-330
...
| Warmup calculation | `full_backtest_runner.py` | 252-263 | `calculate_warmup_days` |
| Coverage gate | `full_backtest_runner.py` | 294-330 | `check_coverage` |
